...
3. Checks `_path` array for loop prevention
...
In `forwardMessage()`, the server ID is appended to the `_path` array:
...
In `fn_parseExternalMessage()`, messages that have already passed through this server are dropped:

```javascript
if (Array.isArray(v_jmsg._path) && v_jmsg._path.includes(getServerOriginID())) {
    return;  // Ignore - this message already passed through us
}
```

### Why This Matters

When `ParentCommServer.forwardMessage()` broadcasts to **all** children, it includes the originating child. Without path tracking:
...
With `_path` check, Child A ignores the bounce-back because its ID is already in the path array.
...
| Parent Server | `fn_parseExternalMessage()` | ✅ | ❌ | Checks `_path` |
| Child Server | `fn_parseExternalMessage()` | ✅ | ❌ | Checks `_path` |
