...
test('fn_buildResponse creates valid response envelope with server ID', () => {
    const { privatePath } = fn_makeTempKeys();
    installFakeGlobals({ s2s_my_private_key: privatePath });

    const nonce = 'abc123';
    const serverId = 'TestServer';
    const response = c_s2s_auth.fn_buildResponse(nonce, serverId);
    const parsed = JSON.parse(response);
    assert.strictEqual(parsed.s2s_auth, 'response');
    assert.strictEqual(parsed.id, serverId);
    assert.strictEqual(typeof parsed.sig, 'string');
    assert.notStrictEqual(parsed.sig, '');
});


// ---------------------------------------------------------------------------
// fn_sign / fn_verify (integration with real keys)
// ---------------------------------------------------------------------------

test('fn_sign and fn_verify work with matching key pair', () => {
    const { privatePath, publicPath } = fn_makeTempKeys();
    installFakeGlobals({
        s2s_my_private_key: privatePath,
        s2s_trusted_server_keys: { test: publicPath }
    });

    const nonce = 'test_nonce_123';
    const signature = c_s2s_auth.fn_sign(nonce);
...
test('fn_getPrivateKey throws when s2s_my_private_key not configured', () => {
    installFakeGlobals({});
    c_s2s_auth.fn_resetKeyCache();
    assert.throws(() => c_s2s_auth.fn_getPrivateKey(), /s2s_my_private_key is not configured/);
});
