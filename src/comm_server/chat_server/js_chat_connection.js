...
function fn_onConnect_Handler(p_ws, p_req) {
    ...
    // Security item 2.1: frame-based WS auth (de_auth frame) is always
    // enabled. The ws_auth_no_frame_old_compatibility flag (default true)
    // controls whether legacy clients that send credentials in the URL
    // query string are still accepted.
    //   true  (default) -> accept backward compatibility (query-string creds
    //                       still work, alongside the frame path).
    //   false           -> do NOT accept backward compatibility; only the
    //                       de_auth frame path is allowed and query-string
    //                       credentials are rejected.
    const c_allowOldCompat = global.m_serverconfig.m_configuration.ws_auth_no_frame_old_compatibility !== false;
    const c_hasQueryCreds = c_params != null
        && c_params.hasOwnProperty(c_CONSTANTS.CONST_CS_LOGIN_TEMP_KEY.toString());

    if (c_hasQueryCreds && !c_allowOldCompat) {
        // Legacy credentials are not accepted — reject and close.
        console.log(global.Colors.BFgYellow + '[WARN] legacy query-string credentials rejected (ws_auth_no_frame_old_compatibility is false)' + global.Colors.Reset);
        if (global.m_logger) global.m_logger.Warn('legacy query-string credentials rejected (ws_auth_no_frame_old_compatibility is false)', 'fn_onConnect_Handler', null, c_params);
        p_ws.close();
        return;
    }

    // If the client sent query creds and old compat is allowed, fall through
    // to the legacy path (with a warning). Otherwise defer to the de_auth frame.
    const c_deferAuthFrame = !c_hasQueryCreds;
    if (c_hasQueryCreds) {
        console.log(global.Colors.BFgYellow + '[WARN] ws_auth_no_frame_old_compatibility is enabled but client used legacy query-string credentials' + global.Colors.Reset);
        if (global.m_logger) global.m_logger.Warn('ws_auth_no_frame_old_compatibility is enabled but client used legacy query-string credentials', 'fn_onConnect_Handler', null, c_params);
    }
    ...
    function fn_validateKey(p_params) {
        ...
        if (p_params.hasOwnProperty(c_CONSTANTS.CONST_CS_LOGIN_TEMP_KEY.toString()) === true) {
            /*
